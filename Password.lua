...
do
    ...
    local Base64 = _G.Base64; -- Make sure it's Base64
...
do
    ...
    function PasswordApi:AddRoot()
        if not fs.exists("/.PasswordApi") then
            fs.makeDir("/.PasswordApi");
        end
        if not fs.exists("/.PasswordApi/usr/") then
    ...
    function PasswordApi:CheckPassForUser(user, pass)
        ...
        return false, "Invaild username or password"
    end

    _G.PasswordApi = PasswordApi;
end